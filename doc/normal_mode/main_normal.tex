...
\color{ao} \verb?electron_isocone_radius? & Creates an isolation cone around electrons with respect to HCAL deposits.\\
\color{ao} \verb?muon_isocone_radius? & Creates an isolation cone around muons with respect to HCAL deposits.\\
\color{ao} \verb?photon_isocone_radius? & Creates an isolation cone around photons with respect to HCAL deposits.\\
\color{ao} \verb?track_isocone_radius? & Creates an isolation cone around tracks with respect to HCAL deposits.\\
...
\color{ao} \verb?cjet_id.status? & Enable advance $c$-jet tagging and mistagging capabilities by setting \verb?on/off?. Default \verb?off?.\\
\color{ao} \verb?cjet_id.exclusive? & Allows several $c$-jets to be issued from a single $C$-hadron. (Only available if \verb?cjet_id.status? is on)\\
\color{ao} \verb?cjet_id.matching_dr? & Angular distance parameter matching a $c$-jet with a $C$-hadron. (Only available if \verb?cjet_id.status? is on)\\
...
\color{ao} \verb?tau_id.misid_ljet? &  Mistagging rate of a light jet as a hadronic tau,
   as a float.\\
   \color{ao} \verb?tau_id.reconstruction_method? & Reconstruction method for hadronic taus. Jet-based reconstruction allows matching truth level taus to the jets. Hadron-based reconstruction finds hadronic taus in the event and uses them as tau objects. Default \verb?hadron-based?.\\
\hline
\end{tabular}
\end{center}
...
The bounds correspond to the domain that the scaling function applies ({\tt PT > ..., ETA < ...}, etc.).
...
\end{verbatim}}
Here {\tt <jet\_id>} stands for the identification of this particular jet definition which can get any value except the same names as samples, \texttt{<clustering algorithm>} refers to the desired clustering algorithm and \texttt{<parameters>} are the specific parameters for the given clustering algorithm. Details about the usage of this module both in normal mode and expert mode can be found in {\tt madanalysis5/tools/SampleAnalyzer/Interfaces/substructure/README.md}.
