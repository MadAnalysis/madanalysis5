...
class ArchitectureInfo:
    ...
    def __eq__(self, other):
        ...
        log.debug("The other   one (number of items=" + str(len(other.__dict__)) + "):")
        log.debug(str(other.__dict__))
        items_ok = list(self.__dict__.keys()) == list(other.__dict__.keys())
        if not items_ok:
            diff = list(set(self.__dict__.keys()) - set(other.__dict__.keys()))
            log.debug(
                "The comparison of categories -> differences detected: " + str(diff)
            )
            return False
        log.debug("The comparison of categorie names -> OK")
        log.debug("The comparison of categorie values:")
        diff = False
        for key in self.__dict__.keys():
            if self.__dict__[key] != other.__dict__[key]:
                log.debug("  -> difference here: " + str(key))
                diff = True
        if not diff:
            log.debug("  -> OK")
    ...
    def save(self, filename):
        ...
        try:
            ...
            log.error("impossible to write the configuration file '" + filename + "'")
            return False
        ...
        try:
            ...
            log.error("error occured during saving data to " + filename)
            test = False
    ...
    def load(self, filename):
        ...
        try:
            ...
            log.error("impossible to read the configuration file '" + filename + "'")
            return False
        ...
        try:
            ...
            log.error("error occured during reading data from " + filename)
            test = False
        ...
        try:
            ...
            log.error("error occured during copying data from " + filename)
            test = False
