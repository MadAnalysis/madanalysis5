...
class MakefileWriter:
    ...
    def UserfriendlyMakefileForSampleAnalyzer(filename, options):
        ...
        file.write(
            "#"
            + StringTools.Center("GENERAL MAKEFILE DEVOTED TO SAMPLEANALYZER", 78)
            + "#\n"
        )
        file.write(StringTools.Fill("#", 80) + "\n")
        file.write("\n")
    ...
    def Makefile(
        ...
        file.write(StringTools.Fill("#", 80) + "\n")
        file.write(
            "#" + StringTools.Center("MAKEFILE DEVOTED TO " + title.upper(), 78) + "#\n"
        )
        file.write(StringTools.Fill("#", 80) + "\n")
        file.write("\n")
        ...
        cxxflags = []
        cxxflags.extend(
            ["-Wall", "-std=c++11", "-O3", "-fPIC", "-I$(MA5_BASE)/tools/"]
        )  # general
        ...
        if (
            not options.has_root_inc
            and not options.has_fastjet_inc
            and not options.has_zlib_inc
            and not options.has_delphes_inc
            and not options.has_delphesMA5tune_inc
        ):
            ...
            if archi_info.compilation_severity >= 2:
                ...
                cxxflags.extend(
                    ["-ansi", "-pedantic"]
                )  # only ANSI compatible code, else rejecting
                cxxflags.extend(
                    ["-Wno-long-long"]
                )  # except 'long long' = 64 bits (mandatory for STDHEP format)
                file.write("CXXFLAGS += " + " ".join(cxxflags) + "\n")
        ...
        if len(libs) != 0:
            ...
            for ind in range(0, len(libs)):
                ...
                file.write(
                    '\t@echo -e $(RED)"The shared library "$(REQUIRED'
                    + str(ind + 1)
                    + ')" is not found"\n'
                )
                file.write(
        ...
        file.write(
            '\t@echo -e "'
            + StringTools.Center("Removing intermediate files from building", 56)
            + '"\n'
        )
        file.write('\t@echo -e "' + StringTools.Fill("-", 56) + '"$(NORMAL)\n')
        file.write("\n")
        ...
        file.write('\t@echo -e $(YELLOW)"' + StringTools.Fill("-", 56) + '"\n')
        file.write(
            '\t@echo -e "' + StringTools.Center("Cleaning all the project", 56) + '"\n'
        )
        file.write('\t@echo -e "' + StringTools.Fill("-", 56) + '"$(NORMAL)\n')
        file.write("\n")
