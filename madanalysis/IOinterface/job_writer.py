...
class JobWriter(object):
    ...
    def CreateJobStructure(path, recastflag):
        if not os.path.isdir(path):
            ...
            try:
                ...
                logging.getLogger("MA5").error(
                    "Impossible to create the folder 'Build/Lib'"
                )
                return False
            try:
                ...
                logging.getLogger("MA5").error(
                    "Impossible to create the folder 'Build/SampleAnalyzer'"
                )
                return False
            try:
                ...
                logging.getLogger("MA5").error(
                    "Impossible to create the folder 'Build/SampleAnalyzer/User'"
                )
                return False
            try:
                ...
                logging.getLogger("MA5").error(
                    "Impossible to create the folder 'Build/SampleAnalyzer/User/Analyzer'"
                )
                return False
            try:
                ...
                logging.getLogger("MA5").error(
                    "Impossible to create the folder 'Build/Log'"
                )
                return False
            try:
                ...
                logging.getLogger("MA5").error(
                    "Impossible to create the folder 'Build/Main'"
                )
                return False
        try:
        ...
        try:
            ...
            logging.getLogger("MA5").error(
                "Impossible to create the folder 'Output/HTML'"
            )
            return False
        try:
            ...
            logging.getLogger("MA5").error(
                "Impossible to create the folder 'Output/Histos'"
            )
            return False
        try:
    ...
    def CheckJobStructure(self, recastflag):
        ...
        if not recastflag:
            if not os.path.isdir(self.path + "/Build"):
                logging.getLogger("MA5").error(
                    "folder '" + self.path + "/Build' is not found"
                )
                return False
            elif not os.path.isdir(self.path + "/Build/Lib"):
                logging.getLogger("MA5").error(
                    "folder '" + self.path + "/Build/Lib' is not found"
                )
                return False
            elif not os.path.isdir(self.path + "/Build/SampleAnalyzer"):
                logging.getLogger("MA5").error(
                    "folder '" + self.path + "/Build/SampleAnalyzer' is not found"
                )
                return False
            elif not os.path.isdir(self.path + "/Build/SampleAnalyzer/User"):
                logging.getLogger("MA5").error(
                    "folder '" + self.path + "/Build/SampleAnalyzer/User' is not found"
                )
                return False
                ...
                logging.getLogger("MA5").error(
                    "folder '"
                    + self.path
                    + "/Build/SampleAnalyzer/User/Analyzer' is not found"
                )
                return False
            elif not os.path.isdir(self.path + "/Build/Main"):
                logging.getLogger("MA5").error(
                    "folder '" + self.path + "/Build/Main' is not found"
                )
                return False
        if not os.path.isdir(self.path + "/Output"):
            logging.getLogger("MA5").error(
                "folder '" + self.path + "/Output' is not found"
            )
            return False
        elif not os.path.isdir(self.path + "/Input"):
            logging.getLogger("MA5").error(
                "folder '" + self.path + "/Input' is not found"
            )
            return False
        elif not os.path.isfile(self.path + "/history.ma5"):
            logging.getLogger("MA5").error(
                "file '" + self.path + "/history.ma5' is not found"
            )
            return False
        else:
            return True
    ...
    def CopyDelphesCard(self, input, output, cfg, theFile):
        ...
        for line in input:
            ...
            if len(words) >= 2:
                if words[0].lower() == "set" and words[1].lower() == "executionpath":
            ...
            if len(words) >= 3:
                if (
                    words[0].lower() == "module"
                    and words[1].lower() == "treewriter"
                    and words[2].lower() == "treewriter"
                ):
                    TagTreeWriter = True
                    if cfg.skim_genparticles:
            ...
            if len(words) >= 5 and TagTreeWriter:
                if words[0].lower() == "add" and words[1].lower() == "branch":
                    if words[3].lower() == "particle" and cfg.skim_genparticles:
                        output.write(
                            "  add Branch MA5Filter/filteredParticles      Particle    GenParticle\n"
                        )
                        continue
    ...
    def CreateDelphesCard(self):
        ...
        try:
            if self.main.fastsim.package == "delphes":
                ...
                input = open(
                    self.main.archi_info.ma5dir
                    + "/tools/SampleAnalyzer/Interfaces/delphesMA5tune/"
                    + cardname,
                    "r",
                )
        except:
    ...
    def CopyLHEAnalysis(self):
        ...
        if not recast:
            try:
                ...
                logging.getLogger("MA5").error(
                    'Impossible to copy the file "newAnalyzer"'
                )
                return False
            try:
                ...
                logging.getLogger("MA5").error(
                    'Impossible to make executable the file "newAnalyzer"'
                )
                return False
    ...
    def CreateHeader(self, file):
        ...
        file.write("//         Jack Araz, Eric Conte and Benjamin Fuks\n")
        file.write(
            "//  Official website: <https://github.com/MadAnalysis/madanalysis5>\n"
        )
        file.write("//  \n")
    ...
    def CreateMainFct(self, file, analysisName, outputName):
        ...
        file.write('  if (!manager.Initialize(argc,argv,"pdg.ma5")) ' + "return 1;\n\n")
        ...
        if self.merging.enable:
            ...
            file.write(
                '  parametersA2["njets"]="' + str(self.main.merging.njets) + '";\n'
            )
            file.write(
                '  parametersA2["ma5_mode"]="'
                + str(int(self.main.merging.ma5_mode))
                + '";\n'
            )
        ...
        if self.output != "" and not self.output.lower().endswith("root"):
            ...
            if self.output.lower().endswith("lhe") or self.output.lower().endswith(
                "lhe.gz"
            ):
                file.write(
                    '      manager.InitializeWriter("lhe","' + self.output + '");\n'
                )
            elif self.output.lower().endswith("lhco") or self.output.lower().endswith(
                "lhco.gz"
            ):
                file.write(
                    '      manager.InitializeWriter("lhco","' + self.output + '");\n'
                )
        ...
        if self.main.fastsim.package == "fastjet":
            ...
            for k, v in sorted(
                six.iteritems(parameters), key=lambda k_v: (k_v[0], k_v[1])
            ):
                file.write(
                    '  parametersC1["' + (k + '"').ljust(30, " ") + '] = "' + v + '";\n'
                )
            for obj in ["electron", "muon", "track", "photon"]:
                if len(getattr(self.main.superfastsim, obj + "_isocone_radius")) != 0:
                    file.write(
                        "  parametersC1["
                        + ('"isolation.' + obj + '.radius"').ljust(30, " ")
                        + '] = "'
                        + ",".join(
                            [
                                str(x)
                                for x in getattr(
                                    self.main.superfastsim, obj + "_isocone_radius"
                                )
                            ]
                        )
                        + '";\n'
            ...
            file.write(
                '      manager.InitializeJetClusterer("'
                + self.main.fastsim.clustering.algorithm
                + '",parametersC1);\n'
            )
            file.write("  if (cluster1==0) return 1;\n\n")
            if len(self.main.jet_collection) > 0:
                # Write configuration for other jets
                for ix, (key, item) in enumerate(
                    self.main.jet_collection.collection.items()
                ):
        ...
        if self.main.fastsim.package in ["delphes", "delphesMA5tune"]:
            ...
            parameters = self.main.fastsim.SampleAnalyzerConfigString()
            for k, v in sorted(
                six.iteritems(parameters), key=lambda k_v1: (k_v1[0], k_v1[1])
            ):
            ...
            if self.main.fastsim.package == "delphes":
                file.write(
                    '      manager.InitializeDetector("delphes","../Input/'
                    + cardname
                    + '",parametersD1);\n'
                )
            else:
                file.write(
                    '      manager.InitializeDetector("delphesMA5tune","../Input/'
                    + cardname
                    + '",parametersD1);\n'
                )
    ...
    def WriteSelectionHeader(self, main):
        ...
        if main.superfastsim.isNewSmearerOn():
            file = open(
                self.path + "/Build/SampleAnalyzer/User/Analyzer/new_smearer_reco.h", "w"
            )
            ...
            file.close()
            if main.superfastsim.isSmearerOn():
                file = open(
                    self.path + "/Build/SampleAnalyzer/User/Analyzer/sigmas.h", "w"
                )
                job.WriteNewSmearerEfficiencies(file)
                file.close()
            if main.superfastsim.isRecoOn():
                file = open(self.path + "/Build/SampleAnalyzer/User/Analyzer/reco.h", "w")
                job.WriteNewRecoEfficiencies(file)
                file.close()
            if main.superfastsim.isScalingOn():
                file = open(
                    self.path + "/Build/SampleAnalyzer/User/Analyzer/scaling.h", "w"
                )
                job.WriteNewScales(file)
                file.close()

        ## Do we need a tagger?
        if main.superfastsim.isTaggerOn():
            file = open(
                self.path + "/Build/SampleAnalyzer/User/Analyzer/new_tagger.h", "w"
            )
            ...
            file.close()
            file = open(
                self.path + "/Build/SampleAnalyzer/User/Analyzer/efficiencies.h", "w"
            )
    ...
    def WriteSelectionSource(self, main):
        ...
        ## Do we need a smearer?
        if main.superfastsim.isRecoSmearerOn():
            file = open(
                self.path + "/Build/SampleAnalyzer/User/Analyzer/new_smearer_reco.cpp",
                "w",
            )
        ...
        if main.superfastsim.isTaggerOn():
            file = open(
                self.path + "/Build/SampleAnalyzer/User/Analyzer/new_tagger.cpp", "w"
            )
    ...
    def WriteMakefiles(self, option="", **kwargs):
        ...
        options.ma5_fastjet_mode = (
            self.main.archi_info.has_fastjet and self.main.archi_info.has_fjcontrib
        )
        ...
        options.has_fastjet_tag = (
            self.main.archi_info.has_root and self.main.archi_info.has_fastjet
        )
        ...
        for line in history.history:
            ...
            for item in items:
                if (
                    ...
                    or item.startswith("display")
                    or item.startswith("history")
                    or item.startswith("open")
                    or item.startswith("resubmit")
                    or item.startswith("shell")
                    or item.startswith("!")
                    or item.startswith("submit")
                ):
                    pass
                else:
                    file.write(item)
