...
class CmdInstall(CmdBase):
    ...
    def untar(self,installdir,tarball,package):
        ...
        os.system("cd "+installdir+" ; tar xzf "+tarball+\
                  " > "+self.main.archi_info.ma5dir+"/tools/"+package+"/untar.log 2>&1")
    ...
    def install_zlib(self):
        ...
        os.system("cd "+packagedir+" ; ./configure --prefix="+self.main.archi_info.ma5dir+"/tools/zlib > "+self.main.archi_info.ma5dir+"/tools/zlib/"+"configuration.log 2>&1")
        
        # Compiling
        logging.info("Compiling the package ...")
        os.system("cd "+packagedir+" ; make -j"+str(ncores)+" > "+self.main.archi_info.ma5dir+"/tools/zlib/"+"compilation.log 2>&1")

        # Copying headers & libraries
        logging.info("Copying headers and libraries into 'tools/zlib' ...")
        os.system("cd "+packagedir+" ; make install > "+self.main.archi_info.ma5dir+"/tools/zlib/"+"installation.log 2>&1")

        # Final check
        logging.info("Checking installation ...")
        if (not os.path.isdir(self.main.archi_info.ma5dir+"/tools/zlib/include")) or \
           (not os.path.isdir(self.main.archi_info.ma5dir+"/tools/zlib/lib")):
            logging.error('package headers and/or libraries are missing.')
            self.display_log('zlib')
            return False

        if not os.path.isfile(self.main.archi_info.ma5dir+'/tools/zlib/include/zlib.h'):
            logging.error("header labeled 'zlib.h' is missing.")
            self.display_log('zlib')
            return False

        if (not os.path.isfile(self.main.archi_info.ma5dir+'/tools/zlib/lib/libz.so')) and \
           (not os.path.isfile(self.main.archi_info.ma5dir+'/tools/zlib/lib/libz.a')):
    ...
    def install_delphes(self):
        ...
        installdir = self.main.archi_info.ma5dir + '/tools/delphes/'
        ...
        os.system("mv "+packagedir+"/* "+self.main.archi_info.ma5dir+"/tools/delphes/")
        packagedir = self.main.archi_info.ma5dir+"/tools/delphes/"
        ...
        # Final check
        logging.info("Checking installation ...")
        if (not os.path.isdir(self.main.archi_info.ma5dir+"/tools/delphes/modules")):
            logging.error('package modules are missing.')
            self.display_log('delphes')
            return False

        if not os.path.isfile(self.main.archi_info.ma5dir+'/tools/delphes/modules/ParticlePropagator.h'):
            logging.error("header labeled 'modules/ParticlePropagator.h' is missing.")
            self.display_log('delphes')
            return False

        if not os.path.isfile(self.main.archi_info.ma5dir+'/tools/delphes/libDelphes.so'):
    ...
    def install_delphesMA5tune(self):
        ...
        # Creating package folder
        if not self.create_package_folder('delphesMA5tune'):
            return False
        ...
        # List of files
#       files = { "delphesMA5tune.tar.gz" : "http://cp3.irmp.ucl.ac.be/downloads/Delphes-3.0.12.tar.gz" }
        files = { "delphesMA5tune.tar.gz" : "http://cp3.irmp.ucl.ac.be/downloads/Delphes-3.1.1.tar.gz"}
        
        # Launching wget
        if not self.wget(files,'delphesMA5tune',installdir):
            return False

        # Detarring package
        packagedir = self.untar(installdir,'delphesMA5tune.tar.gz','delphesMA5tune')
        if packagedir == "":
            return False

        # Changing the name of the directory
        os.system("mv "+packagedir+"/* "+self.main.archi_info.ma5dir+"/tools/delphesMA5tune/")
        packagedir = self.main.archi_info.ma5dir+"/tools/delphesMA5tune/"
        ...
        # Configuring
        logging.info("Configuring the package ...")
        os.system("cd "+packagedir+" ; ./configure > "+self.main.archi_info.ma5dir+"/tools/delphesMA5tune/"+"configuration.log 2>&1")

        # Compiling
        logging.info("Compiling the package ...")
        os.system("cd "+packagedir+" ; make -j"+str(ncores)+" > "+self.main.archi_info.ma5dir+"/tools/delphesMA5tune/"+"compilation.log 2>&1")

        # Final check
        logging.info("Checking installation ...")
        if (not os.path.isdir(self.main.archi_info.ma5dir+"/tools/delphesMA5tune/modules")):
            logging.error('package modules are missing.')
            self.display_log('delphesMA5tune')
            return False

        if not os.path.isfile(self.main.archi_info.ma5dir+'/tools/delphesMA5tune/modules/ParticlePropagator.h'):
            logging.error("header labeled 'modules/ParticlePropagator.h' is missing.")
            self.display_log('delphesMA5tune')
            return False
        ...
        if not os.path.isfile(self.main.archi_info.ma5dir+'/tools/delphesMA5tune/DelphesSTDHEP'):
...
##         if not self.wget(files,'MCatNLO-utilities', self.main.archi_info.ma5dir + '/tools'):
##             return False
        shutil.copy(self.main.archi_info.ma5dir+'/../mcatnlo.tar.gz', self.main.archi_info.ma5dir + '/tools/mcatnlo-utilities.tar.gz')
        logging.warning("clear comments  + shutil copy in install.py")

        # Unpacking package
        packagedir = self.untar(self.main.archi_info.ma5dir + '/tools/MCatNLO-utilities','../'+name, 'MCatNLO-utilities')
        if packagedir == "":
            return False
        os.system('rm -f ' + self.main.archi_info.ma5dir + '/tools/' + name)

        # Configuring the Makefile
        logging.info(" * Configuring the package ...")
        path = self.main.archi_info.ma5dir + '/tools/MCatNLO-utilities/StdHEP/src/make_opts'
        text = open(path).read()
        if sys.maxsize > 2**32:
        ...
        # Compiling
        logging.info(" * Compiling the package ...")
        os.system('cd ' +self.main.archi_info.ma5dir+'/tools/MCatNLO-utilities; make > '\
           + self.main.archi_info.ma5dir + '/tools/MCatNLO-utilities/compilation.log 2>&1')
    ...
    def display_log(self,package):
        ...
        logdir=self.main.archi_info.ma5dir+"/tools/"+package+"/"
    ...
    def install_fastjet(self):
        ...
        # Configuring
        logging.info("Configuring the package ...")
        os.system("cd "+packagedir+" ; ./configure --prefix="+self.main.archi_info.ma5dir+"/tools/fastjet > "+self.main.archi_info.ma5dir+"/tools/fastjet/"+"configuration.log 2>&1")
        
        # Compiling
        logging.info("Compiling the package ...")
        os.system("cd "+packagedir+" ; make -j"+str(ncores)+" > "+self.main.archi_info.ma5dir+"/tools/fastjet/"+"compilation.log 2>&1")

        # Copying headers & libraries
        logging.info("Copying headers and libraries into 'tools/fastjet' ...")
        os.system("cd "+packagedir+" ; make install > "+self.main.archi_info.ma5dir+"/tools/fastjet/"+"installation.log 2>&1")

        # Final check
        logging.info("Checking installation ...")
        if (not os.path.isdir(self.main.archi_info.ma5dir+"/tools/fastjet/include")) or \
           (not os.path.isdir(self.main.archi_info.ma5dir+"/tools/fastjet/lib")) or \
           (not os.path.isdir(self.main.archi_info.ma5dir+"/tools/fastjet/bin")):
            logging.error('package headers and/or libraries are missing.')
            self.display_log('fastjet')
            return False

        if not os.path.isfile(self.main.archi_info.ma5dir+'/tools/fastjet/bin/fastjet-config'):
            logging.error("binary labeled 'fastjet-config' is missing.")
            self.display_log('fastjet')
            return False

        if not os.path.isfile(self.main.archi_info.ma5dir+'/tools/fastjet/include/fastjet/PseudoJet.hh'):
            logging.error("header labeled 'include/fastjet/PseudoJet.hh' is missing.")
            self.display_log('fastjet')
            return False

        if (not os.path.isfile(self.main.archi_info.ma5dir+'/tools/fastjet/lib/libfastjet.so')) and \
           (not os.path.isfile(self.main.archi_info.ma5dir+'/tools/fastjet/lib/libfastjet.a')):
    ...
    def install_samples(self):
        ...
        if os.path.isdir(self.main.archi_info.ma5dir + '/samples'):
            logging.info("'samples' folder is already created")
        else:
            logging.info("Creating the 'samples' folder ...")
            try:
                os.mkdir(self.main.archi_info.ma5dir + '/samples')
            except:
        ...
        for file in files.keys():
            ...
            if os.path.isfile(self.main.archi_info.ma5dir + '/samples/' + file):
                logging.info("Removing file '" + file + "' ...")
                try:
                    os.remove(self.main.archi_info.ma5dir + '/samples/' + file)
                except:
...
#            try:
            urllib.urlretrieve(url,self.main.archi_info.ma5dir+'/samples/'+file,CmdInstall.reporthook)
#            except:
