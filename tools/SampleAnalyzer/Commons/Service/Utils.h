...
namespace MA5
{
    ...
    template<class Type>
    std::vector<Type> filter(std::vector<Type> objects,
                             MAfloat64 ptmin,
                             MAfloat64 absetamax=20.,
                             MAfloat64 absetamin=-1.)
    {
    ...
    std::vector<Type> filter_select(std::vector<Type> objects, FN func)
    {
        std::vector<Type> filtered;
        for (auto & obj: objects)
    ...
    // Sorting //
    //=========//
    template<class Type> void PTsorted(std::vector<Type> &objects)
    {
        std::sort(objects.begin(), objects.end(),
            [] (Type a, Type b) {return a.pt() > b.pt();});
    }

    template<class Type> void Esorted(std::vector<Type> &objects)
    {
        std::sort(objects.begin(), objects.end(),
            [] (Type a, Type b) {return a.e() > b.e();});
    }
    ...
    template<typename T1, typename T2> std::vector<T1>
      OverlapRemoval(std::vector<T1> &v1, std::vector<T2> &v2,
      const MAdouble64 &drmin)
    {
      // Determining with objects should be removed
      std::vector<bool> mask(v1.size(),false);
    ...
    // Example:  signaljets = conditional_removal(signaljets,signalel,
    //            [] (RecJetFormat jet, RecLeptonFormat el) {return jet.dr(el) > 0.2;});
    template<typename T1, typename T2, typename FN>
    std::vector<T1> conditional_removal(std::vector<T1> &v1, std::vector<T2> &v2, FN func)
    {
        // Determining with objects should be removed
        std::vector<bool> mask(v1.size(),false);
        for (MAuint32 j=0;j<v1.size();j++)
