...
void SampleAnalyzer::CheckDatatypes() const
{
	...
	try
	{
		value = sizeof(MAint8);
		if (value != 1)
			throw EXCEPTION_WARNING("MAint8 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	...
	try
	{
		...
		if (value != 1)
			throw EXCEPTION_WARNING("MAuint8 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	...
	try
	{
		...
		if (value != 2)
			throw EXCEPTION_WARNING("MAint16 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
	...
	try
	{
		...
		if (value != 2)
			throw EXCEPTION_WARNING("MAuint16 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
	...
	try
	{
		...
		if (value != 4)
			throw EXCEPTION_WARNING("MAint32 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
	...
	try
	{
		...
		if (value != 4)
			throw EXCEPTION_WARNING("MAuint32 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
	...
	try
	{
		...
		if (value != 8)
			throw EXCEPTION_WARNING("MAint64 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
	...
	try
	{
		...
		if (value != 8)
			throw EXCEPTION_WARNING("MAuint64 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
	...
	try
	{
		...
		if (value != 4)
			throw EXCEPTION_WARNING("MAfloat32 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
	...
	try
	{
		...
		if (value != 8)
			throw EXCEPTION_WARNING("MAfloat64 type corresponds to " + CONVERT->ToString(value * 8) + " bits", "", 0);
	}
	catch (const std::exception &e)
	{
		MANAGE_EXCEPTION(e);
...
MAbool SampleAnalyzer::Initialize(MAint32 argc, MAchar **argv, const std::string &pdgFileName)
{
	...
	if (pos != std::string::npos)
		datasetName_.resize(pos);
	pos = datasetName_.rfind('/');
	if (pos != std::string::npos)
		datasetName_.erase(0, pos + 1);
...
MAbool SampleAnalyzer::CreateDirectoryStructure()
{
	...
	if (pos != std::string::npos)
		dirname = "../Output/SAF/" + dirname.substr(pos + 1);
	else
		dirname = "../Output/SAF/" + dirname;
	size_t found = dirname.find(".list");
	if (found != std::string::npos)
		dirname.replace(found, 5, "");
	if (CreateDir(dirname) == -1)
	{
		return false;
	}
	...
	for (MAuint32 i = 0; i < analyzers_.size(); i++)
	{
		...
		for (MAuint32 ii = 0; check != 0; ii++)
		{
			...
			{
				analyzers_[i]->SetOutputDir(newdirname + "_" + ss.str());
			}
		}

		// Creating one suybdirectory for the histograms and another one for the cutflow
		if (CreateDir(analyzers_[i]->Output() + "/Histograms") == -1)
		{
			return false;
		}
		if (CreateDir(analyzers_[i]->Output() + "/Cutflows") == -1)
		{
			return false;
		}
	}
...
StatusCode::Type SampleAnalyzer::NextFile(SampleFormat &mySample)
{
	...
	else
	{
		...
		std::stringstream str;
		if (unit == 1)
			str << static_cast<MAuint32>(value);
		else
...
StatusCode::Type SampleAnalyzer::NextEvent(SampleFormat &mySample, EventFormat &myEvent)
{
	...
	if (test == StatusCode::KEEP)
	{
		...
		if (!myReader_->FinalizeEvent(mySample, myEvent))
			return StatusCode::SKIP;
	...
	// FAILURE case
	else if (test == StatusCode::FAILURE)
		return test;
