...
StatusCode::Type DelphesTreeReader::ReadEvent(EventFormat &myEvent, SampleFormat &mySample)
{
    ...
    if (read_nevents_ >= total_nevents_)
...
void DelphesTreeReader::FillEvent(EventFormat &myEvent, SampleFormat &mySample)
{
    ...
    if (data_.Electron_ != 0)
    {
        ...
        for (MAuint32 i = 0; i < nelectrons; i++)
        {
            ...
            if (part->Charge > 0)
                electron->charge_ = true;
            else
                electron->charge_ = false;
            ...
            if (mc != 0)
            {
                ...
                if (genit != gentable.end())
                    electron->mc_ = &(myEvent.mc()->particles()[genit->second]);
                else
                    WARNING << "GenParticle corresponding to an electron is not found in the gen table" << endmsg;
            }
    ...
    if (data_.Photon_ != 0)
    {
        ...
        for (MAuint32 i = 0; i < nphotons; i++)
        {
            ...
            if (mc != 0)
            {
                ...
                genit = gentable.find(mc);
                if (genit != gentable.end())
                    photon->mc_ = &(myEvent.mc()->particles()[genit->second]);
                else
                    WARNING << "GenParticle corresponding to a photon is not found in the gen table" << endmsg;
            }
        }
    }
    ...
    if (data_.Muon_ != 0)
    {
        ...
        for (MAuint32 i = 0; i < nmuons; i++)
        {
            ...
            if (mc != 0)
            {
                genit = gentable.find(mc);
                if (genit != gentable.end())
                    muon->mc_ = &(myEvent.mc()->particles()[genit->second]);
                else
                    WARNING << "GenParticle corresponding to a muon is not found in the gen table" << endmsg;
            }
    ...
    if (data_.Track_ != 0)
    {
        ...
        for (MAuint32 i = 0; i < ntracks; i++)
        {
            ...
            if (mc != 0)
            {
                genit = gentable.find(mc);
                if (genit != gentable.end())
                    track->mc_ = &(myEvent.mc()->particles()[genit->second]);
                else
                    WARNING << "GenParticle corresponding to a track is not found in the gen table" << endmsg;
            }
                ...
                if (mc != 0)
                {
                    ...
                    try
                    {
                        if (genit != gentable.end())
                            ...
                            throw EXCEPTION_WARNING("GenParticle corresponding to a vertex is not found in the gen table", "", 1);
                    }
