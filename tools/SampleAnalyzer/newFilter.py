...
class FilterManager:
    ...
    def WriteHeader(self):
        ...
        file.write('{\n')
        file.write('  INIT_FILTER('+self.name+',"'+self.title+'")\n\n')
        file.write(' public:\n')
        file.write('  virtual bool Initialize(const MA5::Configuration& cfg, const std::map<std::string,std::string>& parameters);\n')
        file.write('  virtual void Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files, SAFWriter& out);\n')
    ...
    def WriteSource(self):
        ...
        file.write('// -----------------------------------------------------------------------------\n')
        file.write('void '+self.name+'::Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files, SAFWriter& out)\n')
        file.write('{\n')
        ...
        file.write('// and returning a veto on the current event (true=event kept, false=event rejected)\n')
        file.write('// -----------------------------------------------------------------------------\n')
