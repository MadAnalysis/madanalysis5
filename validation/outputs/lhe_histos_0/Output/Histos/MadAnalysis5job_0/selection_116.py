def selection_116():
    ...
    pad.hist(x=xData, bins=xBinning, weights=y117_P_0_weights,\
    ...
    plt.xlabel(r"$p$ $[ j ]$ $(GeV/c)$ ",\
               fontsize=16,color="black")
    plt.ylabel(r"$\mathrm{N.}\ \mathrm{of}\ j$ $(\mathrm{not}\ \mathrm{normalized})$",\
               fontsize=16,color="black")

    # Boundary of y-axis
    ymax=(y117_P_0_weights).max()*1.1
    ymin=0 # linear scale
    #ymin=min([x for x in (y117_P_0_weights) if x])/100. # log scale
    ...
    # Saving the image
    plt.savefig('../../HTML/MadAnalysis5job_0/selection_116.png')
    plt.savefig('../../PDF/MadAnalysis5job_0/selection_116.png')
    plt.savefig('../../DVI/MadAnalysis5job_0/selection_116.eps')
