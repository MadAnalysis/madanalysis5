def selection_61():
    ...
    pad.hist(x=xData, bins=xBinning, weights=y62_ETA_0_weights,\
    ...
    plt.xlabel(r"$\eta$ $[ mu_{1} ]$ ",\
               fontsize=16,color="black")
    plt.ylabel(r"$\mathrm{Events}$ $(\mathrm{not}\ \mathrm{normalized})$",\
               fontsize=16,color="black")

    # Boundary of y-axis
    ymax=(y62_ETA_0_weights).max()*1.1
    ymin=0 # linear scale
    #ymin=min([x for x in (y62_ETA_0_weights) if x])/100. # log scale
    ...
    # Saving the image
    plt.savefig('../../HTML/MadAnalysis5job_0/selection_61.png')
    plt.savefig('../../PDF/MadAnalysis5job_0/selection_61.png')
    plt.savefig('../../DVI/MadAnalysis5job_0/selection_61.eps')
