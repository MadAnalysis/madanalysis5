def selection_30():
    ...
    pad.hist(x=xData, bins=xBinning, weights=y31_PHI_0_weights,\
    ...
    plt.xlabel(r"$\phi$ $[ e_{1} ]$ ",\
               fontsize=16,color="black")
    plt.ylabel(r"$\mathrm{Events}$ $(\mathrm{not}\ \mathrm{normalized})$",\
               fontsize=16,color="black")

    # Boundary of y-axis
    ymax=(y31_PHI_0_weights).max()*1.1
    ymin=0 # linear scale
    #ymin=min([x for x in (y31_PHI_0_weights) if x])/100. # log scale
